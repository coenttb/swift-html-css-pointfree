...
struct BlockQuoteTests {
    ...
    func blockquoteWithFooterAndCiteRendersCorrectly() {
        assertInlineSnapshot(
            of: BlockQuote {
                Paragraph {
                    HTMLText("The future belongs to those who believe in the beauty of their dreams.")
                }
                Footer {
                    HTMLText("— ")
                    Cite {
    ...
    func multiParagraphBlockquoteRendersCorrectly() {
        assertInlineSnapshot(
            of: BlockQuote {
                Paragraph {
                    HTMLText("To be yourself in a world that is constantly trying to make you something else is the greatest accomplishment.")
                }
                Paragraph {
                    HTMLText("Do not go where the path may lead, go instead where there is no path and leave a trail.")
                }
            },
            as: .html
        ) {
    ...
    func blockquoteWithCustomStylingRendersCorrectly() {
        assertInlineSnapshot(
            ...
            .attribute("style", "font-style: italic; border-left: 4px solid #ccc; padding-left: 20px;"),
            as: .html
        ) {
            """
    ...
    func nestedBlockquoteRendersCorrectly() {
        assertInlineSnapshot(
            of: BlockQuote {
                ...
                BlockQuote {
                    HTMLText("The best time to plant a tree was 20 years ago. The second best time is now.")
                }
